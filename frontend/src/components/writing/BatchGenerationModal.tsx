...
function formatProjectTaskStreamStatus(status: "idle" | "connecting" | "open" | "error"): string {
  ...
  if (status === "error") return "已切回轮询";
  return "未连接";
}

function buildStepLabel(item: BatchGenerationTaskItem): string {
  const requestId = item.last_request_id ? ` | 定位编号 ${item.last_request_id}` : "";
  const attempts = ` | 尝试 ${item.attempt_count}`;
  ...
  return (
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <div>
          ...
          <div className="mt-1 text-xs text-subtext">
            批量起草只会先生成候选结果与记录。在你把结果回填进编辑器前，章节正文不会被直接改写。
          </div>
        </div>
        <button
      ...
      <div className="mt-3 flex flex-wrap items-center justify-between gap-2 rounded-atelier border border-border bg-canvas px-3 py-2 text-xs text-subtext">
        <span aria-label="batch_generation_live_status">实时连接: {streamStatusLabel}</span>
      ...
      <div className="mt-4 grid gap-3">
        <div className="grid gap-2 rounded-atelier border border-border bg-canvas p-3">
          <div className="text-sm font-medium text-ink">第一步：起草范围</div>
          <div className="text-xs text-subtext">
            从这里之后开始：{props.activeChapterNumber ? `第 ${props.activeChapterNumber} 章之后` : "第 1 章之后"}
          </div>
          <div className="text-[11px] text-subtext">
            当前章节会决定起点，系统会从它之后按顺序继续起草。
          </div>
        </div>

        <div className="grid gap-2 rounded-atelier border border-border bg-canvas p-3">
          <div className="text-sm font-medium text-ink">第二步：起草选项</div>
          <div className="flex flex-wrap items-end gap-3">
            <label className="grid gap-1">
              <span className="text-xs text-subtext">章节数：1~200</span>
              <input
        ...
        <div className="grid gap-3 rounded-atelier border border-border bg-surface p-3">
          <div className="flex flex-wrap items-center justify-between gap-2">
            <div className="text-sm font-medium text-ink">第三步：启动与恢复</div>
            <div className="flex flex-wrap items-center gap-2">
              ...
              {canCancel ? (
                <button
                  ...
                  type="button"
                >
                  {props.batchLoading ? "处理中..." : "取消本轮批量起草"}
                </button>
              ) : null}
            </div>
          </div>
          ...
          {!task ? (
            <FeedbackEmptyState
              variant="compact"
              title="还没有批量生成任务"
              description="先设定生成范围和策略，确认后再启动一轮批量起草。"
            />
          ) : null}
            ...
            <>
              ...
              >
                <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-ink">
                  <span>
                    当前状态：<span className="font-mono">{task.status}</span>
                  </span>
                  <span>
                ...
                <div className="text-[11px] text-subtext">
                  已请求暂停：{String(task.pause_requested)} | 已请求取消：{String(task.cancel_requested)}
                </div>
                {requestId ? <div className="text-[11px] text-subtext">定位编号: {requestId}</div> : null}
              </section>

              {latestCheckpoint ? (
                <section className="rounded-atelier border border-border bg-canvas p-3">
                  <div className="text-sm text-ink">恢复节点</div>
              ...
              {task.error_json ? (
                <FeedbackCallout tone="danger" title="批量生成任务返回错误">
                  <div className="break-words text-xs">{task.error_json}</div>
                </FeedbackCallout>
              ) : null}
              ...
              >
                {timeline.length === 0 ? (
                  <FeedbackEmptyState
                    ...
                    variant="compact"
                    title="还没有运行事件"
                    description="任务开始推进后，这里会显示排队、重试、暂停和恢复等时间线。"
                  />
                ) : (
              ...
              >
                {props.batchItems.length === 0 ? (
                  <FeedbackEmptyState
                    ...
                    variant="compact"
                    title="还没有章节执行项"
                    description="任务启动后，这里会按章节列出每一项的状态、错误和可回填结果。"
                  />
                ) : (
                        ...
                        <div className="min-w-0 flex-1">
                          ...
                          {item.started_at || item.finished_at ? (
                            <div className="mt-1 text-[11px] text-subtext">
                              开始于: {item.started_at || "-"} | 结束于: {item.finished_at || "-"}
                            </div>
                          ) : null}
                        </div>
